...
function LapisInput (props: ILapisInputProps, ref: React.RefObject<ILapisInputRef>&any) {
    ...
    const handlerFocus = React.useCallback((e:React.FormEvent<HTMLInputElement>)=>{
    ...
    const handlerBlur = React.useCallback((e:React.FormEvent<HTMLInputElement>)=>{
        const v = e.currentTarget.value;
        //
        if(v.length === 0){
            setStatus('');
    ...
    //
    return (
        <div className={`lapis-ui lapis-input ${status}`}>
            <div className='title'>
                <div>
                    {props.title}
                </div>
...
export default React.memo(
    React.forwardRef<ILapisInputRef, ILapisInputProps>(LapisInput)
);